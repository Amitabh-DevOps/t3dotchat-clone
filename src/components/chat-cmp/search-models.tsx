...
export default function SearchModels() {
  ...
  const processedModels: ProcessedModel[] = (openRouterModels?.data || []).map((model: any) => {
    ...
    return {
      ...
      badge: model.tier === 'premium' ? <Gem className="size-3 text-muted-foreground" /> : null,
          ...
          {isAll ? (
            ...
            filteredModels.map((model) => (
              <div
                ...
                role="menuitem"
                className={cn(
                  "relative select-none rounded-sm text-sm outline-none transition-colors focus:bg-accent/30 focus:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 flex flex-col items-start gap-1 p-3",
                  model.isActive ? "cursor-pointer hover:bg-accent/30" : "cursor-not-allowed hover:!bg-transparent opacity-70"
                )}
                onClick={() => model.isActive && handleModelSelect(model.id)}
              >
                <div className="flex w-full overflow-hidden items-center justify-between">
                  <div className={cn(
                    "flex text-nowrap truncate items-center gap-2 pr-2 font-medium transition-colors",
                    model.isActive ? "text-foreground" : "text-muted-foreground opacity-50"
                  )}>
                    {model.icon}
